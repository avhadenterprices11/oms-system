...
export function ChannelChat({ channelName = "general" }: ChannelChatProps) {
  const [message, setMessage] = useState("");
  const [messages] = useState([
    { id: 1, sender: "Sarah Miller", text: "Hey team! Just pushed the latest API changes to staging.", time: "10:32 AM", avatar: "SM" },
    { id: 2, sender: "Michael Chen", text: "Thanks! I'll review them now.", time: "10:34 AM", avatar: "MC" },
    { id: 3, sender: "Elena Rodriguez", text: "Can someone help me debug the auth flow? Getting a 401 error.", time: "10:35 AM", avatar: "ER" },
    { id: 4, sender: "You", text: "I can take a look. What endpoint are you hitting?", time: "10:36 AM", avatar: "You", isMe: true },
    { id: 5, sender: "Elena Rodriguez", text: "/api/v2/auth/token - seems like the refresh logic isn't working", time: "10:37 AM", avatar: "ER" },
    { id: 6, sender: "You", text: "Check if you're passing the refresh token in the Authorization header. It should be Bearer {token}", time: "10:38 AM", avatar: "You", isMe: true },
  ]);
      ...
      <div className="border-b border-[#e2e8f0] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#ede9fe] rounded-lg flex items-center justify-center">
            <span className="text-[#5048e5] text-[18px] font-bold">#</span>
          </div>
          <div>
            <h2 className="font-['Inter:Bold',sans-serif] font-bold text-[18px] text-[#0f172a]">
              {channelName}
            </h2>
            <p className="text-[12px] text-[#64748b]">42 members · Engineering team</p>
          </div>
        </div>
      </div>
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
            {!msg.isMe && (
              ...
              >
                <span className="text-white text-[13px] font-bold">{msg.avatar}</span>
              </div>
            )}
